...
export default function DriverNotificationsPage() {
  ...
  const handleRead = (id: string, action: string | null, bookingId: number | null) => {
    ...
    if (action === 'view_trip' && bookingId) navigate(`/driver/trips/${bookingId}`)
    else if (action === 'view_wallet') navigate('/driver/wallet')
  }
    ...
    <div className="w-full flex flex-col">
      ...
      {!isLoading && notifications.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 px-4">
          ...
          <p className="text-neutral-gray text-xs mt-1 text-center">Thông báo về cuốc xe sẽ hiện ở đây</p>
        </div>
      )}
